...
def skip_without_docker(request, run_without_docker):
    if request.node.get_closest_marker('skip_without_docker') and run_without_docker:
        pytest.skip('skipped without docker')


@pytest.fixture(autouse=True)
def once_without_docker(request, run_without_docker):
    if request.node.get_closest_marker('once_without_docker') and run_without_docker:
